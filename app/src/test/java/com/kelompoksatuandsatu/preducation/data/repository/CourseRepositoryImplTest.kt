...
class CourseRepositoryImplTest {
    ...
    fun `get course home, result loading`() {
        ...
        runTest {
            coEvery { dataSource.getCourseHome(any()) } returns mockCourseHome
    ...
    fun `get course home, result success`() {
        ...
        runTest {
            coEvery { dataSource.getCourseHome(any()) } returns fakeCourseHomeResponse
            courseRepo.getCourseHome("Android").map {
                delay(100)
                it
            }.test {
                ...
                coVerify { dataSource.getCourseHome(any()) }
            }
        }
    }
    ...
    fun `get course home, result error`() {
        runTest {
            coEvery { dataSource.getCourseHome(any()) } throws IllegalStateException("Error")
            courseRepo.getCourseHome("Android").map {
                delay(100)
                it
            }.test {
    ...
    fun `get course topic, result loading`() {
        val mockCourseHome = mockk<CourseResponse>()
        runTest {
            coEvery { dataSource.getCourseTopic(any()) } returns mockCourseHome
    ...
    fun `get course topic, result success`() {
        ...
        runTest {
            coEvery { dataSource.getCourseTopic(any()) } returns fakeCourseHomeResponse
            courseRepo.getCourseTopic("Free").map {
                delay(100)
                it
            }.test {
                ...
                coVerify { dataSource.getCourseTopic(any()) }
            }
        }
    }
    ...
    fun `get course topic, result error`() {
        runTest {
            coEvery { dataSource.getCourseTopic(any()) } throws IllegalStateException("Error")
